...
static void create_pdentry(uint32_t index, void* ptaddr, uint16_t flags){
	if((uint32_t) ptaddr % 4096 != 0){
		panic("EEE");
	}
	page_directory[index] = ((uint32_t)ptaddr & ~0xFFF) | flags | 1;
}
...
void map_page(void* vaddr, void* paddr, uint16_t flags){
	...
	if((uint32_t) vaddr % 4096 != 0 || (uint32_t) paddr % 4096 != 0) panic("Non aligned addresses");
	...
	if((page_directory[pdindex] & 1) == 0){
		create_pdentry(pdindex, (void*)pt, 0x3);		
	}		
